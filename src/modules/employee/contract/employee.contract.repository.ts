...
export class EmployeeContractRepository implements IEmployeeContractRepository {
  ...
  async create(data: EmployeeContractForm, conn: Connection): Promise<boolean> {
    try {
      await conn.query(
        ...
        [data.karyawan_id, data.jenis, data.tgl_kontrak, data.tgl_berakhir, data.total_kontrak],
      );
  ...
  async update(id: BaseEmployee["id"], data: EmployeeContractForm): Promise<boolean> {
      try {
        await pool.query(
          `UPDATE kontrak_karyawan SET jenis = ?, tgl_kontrak = ?, tgl_berakhir = ?, total_kontrak = ? WHERE id = ?`,
          [data.jenis, data.tgl_kontrak, data.tgl_berakhir, data.total_kontrak, id]
        )
