...
import { JamAbsenForm, JamAbsenFormDB, JamAbsenFormSchema, JamAbsenTableSchema, RawJamAbsenTableSchema } from "./jamAbsen.schema";
...
export class JamAbsenService {
  ...
  async getBaseJamAbsen() {
    try {
      const jamAbsen = await this.jamAbsenRepository.getAll();

      return RawJamAbsenTableSchema.array().parse(jamAbsen)
    } catch (error: unknown) {
      console.error("JamAbsenService.getBaseJamAbsen error:", error);
