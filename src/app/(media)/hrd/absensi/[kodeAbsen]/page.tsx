...
import { getAbsenDetailsAction } from '@/features/absensi/AbsensiAction';
...
const AbsensiDetails = async ({ params }: { params: { kodeAbsen: string } }) => {
  ...
  const absensi = await getAbsenDetailsAction(kodeAbsen);
  
  return (
    <>
    <PageTitle>Detail Absensi</PageTitle>
    <div className='page-container-border bg-white rounded p-2 pt-4'>
      <AbsensiDetailsPage absensiDetailsTable={absensi.data ?? []}/>
    </div>
    </>
  )
}
