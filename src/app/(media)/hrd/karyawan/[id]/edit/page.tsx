...
import { getKaryawanFormOptionsAction, getKaryawanForUpdateAction } from '@/features/karyawan/KaryawanAction';
...
const EditKaryawan = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const [formOptions, karyawan] = await Promise.all([
    getKaryawanFormOptionsAction(),
    getKaryawanForUpdateAction(id),
  ])
